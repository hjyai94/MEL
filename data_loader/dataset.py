...
def dense_image_warp(im, dx, dy, interp=cv2.INTER_LINEAR):
    ...
    remapped = cv2.remap(im, map_x, map_y, interpolation=interp,
    ...
    def __call__(self, sample):
        ...
        if self.random_augment:
            ...
            for i in range(4):  # number of channels
                image[:, :, :, i] = image[:, :, :, i] + np.random.uniform(-MAX_INTENSITY_SHIFT,
                                                                          MAX_INTENSITY_SHIFT)  # assumes unit std derivation
